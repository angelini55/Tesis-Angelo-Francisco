...
que coincide con el pasador"""
    ...
    def vent_temp(self):
        window = Toplevel()
        window.title("Calculos de aumento de temperatura")
        vent = TemperaturaWindow(window)
        ...
        DintM = float(self.textos[1].get())/2
        Dintm = float(self.textos[2].get())/2
        #t = self.textos[15].get()
        if unidades == "Sistema Internacional":
            vent.textos2[0].insert(0,DintM/1000)
            vent.textos2[1].insert(0,Dintm/1000)
        else:
            vent.textos2[0].insert(0,DintM)
            vent.textos2[1].insert(0,Dintm)
    ...
    def create_widget(self):
        ...
        self.raio_lbl = Label(self, text="Método de análisis", bg="#A9A9A9", font=("Tahoma", 9))
        ...
        self.labels = []
        textoslbl = ["T: Par de torsión del freno o embrague","D: Diámetro externo del cono de fricción","d: Diámetro interno del cono de fricción",
        "α: Ángulo del cono","µ: Coeficiente de fricción dinámico del material de fricción"]
        ...
        self.labels2 = []
        textoslabels2 = ["Padm: Presión máxima admisible ejercida sobre el material de fricción","F: Fuerza de accionamiento","FD: Factor de diseño"]
        ...
        i=200
        for parameters in self.listunds2:
            parameters.place(x=1020, y=i)
            i += 30

        self.base_path = pathlib.Path(__file__).parent.parent.resolve()
        self.image_filename =resource_path('images\\Freno Conico.png')
        ...
        self.boton1.place(x=450, y=400, width=50, height=40)
        ...
        Internacional = ["N.mm","mm","mm","°","","MPa","N",""]
        ...
        def Mat(event):
            ...
            if self.list5.get() == self.lista_humedo_seco[0]:
                if self.list2.get() == self.listaUnds[0]:
                    ...
                    self.textos[4].insert(0,coeficiente_dato)
                    self.textos[4]["state"]="disabled"
...
if __name__ == "__main__":
    root = Tk()
    root.wm_title("Frenos conico")
    ConicoWindow(root).mainloop()