...
class ZapataExternaWindow_diseño(Frame):
    ...
    def guardar_calc(self):
        a = self.textos2[0].get()
        c = self.textos2[1].get()
        D = self.textos2[2].get()
        b = self.textos2[3].get()
        Θ1 = self.textos2[4].get()
        Θ2 = self.textos2[5].get()

        F = self.textos3[1].get()
        FD = self.textos3[2].get()
        T = self.textos3[4].get()
        self.contenido = "a: {} \nc:{} \nD: {} \nb: {} \nΘ1: {} \nΘ2: {} \nF: {}\nFD: {} \nT: {}".format(a,c,D,b,Θ1,Θ2,F,FD,T)
    ...
    def ventana_analisis(self):
        new_window = ZapataExternaWindow(Toplevel())
        self.caja = Text(new_window)
        self.caja.place(x=750, y=550, height=150, width=100)
        self.caja.insert(1.0,self.Caja1.get(1.0,"end-1c"))

    def ventana_analisis2(self):
        new_window = ZapataExternaWindow(Toplevel())
        self.caja = Text(new_window)
        self.caja.place(x=750, y=550, height=150, width=100)
        self.caja.insert(1.0,self.Caja2.get(1.0,"end-1c"))

    def ventana_analisis3(self):
        new_window = ZapataExternaWindow(Toplevel())
        self.caja = Text(new_window)
        self.caja.place(x=750, y=550, height=150, width=100)
        self.caja.insert(1.0,self.Caja3.get(1.0,"end-1c"))
    ...
    def calc_diseño(self):
        try:
            ...
            a = float(self.textos2[0].get())
            c = float(self.textos2[1].get())
            D = float(self.textos2[2].get())
            b = float(self.textos2[3].get())
            Θ1 = float(self.textos2[4].get())
            Θ2 = float(self.textos2[5].get())
            ...
            I1 = -cos(radians(Θ2)) + cos(radians(Θ1))
            I2 = 0.5*(sin(radians(Θ2))**2) - 0.5*(sin(radians(Θ1))**2)
            I3 = (radians(Θ2/2))-0.25*sin(2*(radians(Θ2))) - ((radians(Θ1/2))-0.25*sin(2*(radians(Θ1))))

            Padm = Pa/FD

            T = (µ*Padm*b*((D/2)**2)*I1)/(sin(radians(Θa)))
            T = round(T,2)
            if T < Tmax:
            ...
            Mn = (Padm*b*a*(D/2)/(sin(Θa*(pi/180))))*I3
            Mf = (µ*Padm*b*(D/2)/(sin(Θa*(3.14/180))))*((D/2)*I1 - a*I2)
            ...
            self.textos3[0].insert(0,Θa)

            self.textos3[1].delete(0,"end")
            self.textos3[1].insert(0,F)
    ...
    def create_widget(self):
        ...
        self.image_filename = resource_path('images\\Zapata externa real.png')
        ...
        self.lbl_img1.place(x=1150, y=320, width=250, height=250)

        Internacional = ["rpm","N","","","horas","kg.m^2/s","N.mm","m/s","mm","mm","mm","mm","°","°","°","N","","MPa","N.mm"]
        Ingles = ["rpm","lb","","","horas","lb.ft^2/s","lb.in","ft/min","in","in","in","in","°","°","°","lb","","PSI","lb.in"]
        ...
        self.labels3 = []
        textoslabels3 = ["Θa: Ángulo donde se aplica la presión máxima admisible","F: Fuerza de accionamiento","Factor de diseño",
        ...
        self.importar_analisis2 = Button(self, text="Importar", command=self.ventana_analisis2)
        self.importar_analisis2.place(x=865, y=350)

        self.importar_analisis3 = Button(self, text="Importar", command=self.ventana_analisis3)
        self.importar_analisis3.place(x=985, y=350)

if __name__ == "__main__":
    root = Tk()
    root.wm_title("Calculos de diseño para frenos y embragues de zapata externa")
    ...
    ZapataExternaWindow_diseño(root).mainloop()